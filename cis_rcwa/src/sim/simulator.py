...
# ---------------------------------------------------------------- 수집효율 기본값
# η(z) = η0·(1 − r0·exp(−z/ld))  — 광학 QE 를 소자 QE 로 옮기는 보정.
#   η0 : 깊이 무관 손실(DTI 측벽 재결합 + 벌크)
#   r0 : 후면 표면 dead-layer.  η(0)=η0(1−r0)
#   ld : 표면항 도달 깊이
#
# 기본값이 '보정 없음(η≡1)' 인 이유 — 실측으로 확인한 것:
#   한때 η0=0.945, r0=0.05 를 기본값으로 뒀는데, 그건 예시용 si 데이터가 과흡수
#   (450nm k=0.25, 흡수깊이 0.14µm)라 파랑이 표면에 몰려 생긴 겉보기 손실을
#   메우던 값이었다. 실측 si(450nm 부근 k≈0.077, 흡수깊이 0.47µm)로 바꾸자
#   그 손실이 사라졌다 — 같은 실측 QE 19점으로 재적합하면 η0=0.995, r0=0.00,
#   즉 '보정 불필요' 가 나온다(on-band 상대오차 η0=1.00 → 2.73%, 0.945 → 3.96%
#   으로 오히려 보정할수록 나빠진다).
#   실제로 실측 QE 는 광학 QE 와 거의 같거나(450nm B: 광학 58.8 / 실측 59.0)
#   광학보다 높아, 수집손실을 넣을 여지 자체가 없다. 잘 패시베이션된 BSI 에서
#   내부양자효율이 1 에 가깝다는 것과도 부합한다.
#
#   -> 후면이 Al2O3/HfOx 로 패시베이션된 구조: r0=0 (표면 손실 없음)
#   -> 패시베이션이 없는 구조: 기존 관행값 0.35 유지 (해당 조건 실측 없음)
#   물질 데이터가 실측이 아닐 때 이 값들로 QE 를 맞추려 들면, 물질 오차를
#   수집효율에 떠넘기게 된다. 물질부터 실측으로 맞출 것.
ETA0_DEFAULT = 1.0
R0_PASSIVATED = 0.0
R0_BARE = 0.35
