...
class RCWAPlaneWaveSimulator:
    ...
    def run(self, wavelength, theta=0.0, phi=0.0, pol_te=1.0, pol_tm=0.0,
        ...
        if pixel_qe and self._pix is not None and self.n_si_layers > 0:
            ...
            # ---- QE 정의: '전체 Si 흡수' = 유한 밴드 흡수(A_band) + 심부 반무한 Si 흡수(T_deep) ----
            # 반무한 기판도 Si(k>0)라 T_deep 은 결국 전부 Si 에서 흡수 -> external QE.
            # 심부 흡수는 밴드 바닥 투과 flux 맵으로 픽셀 귀속 (그 깊이엔 DTI 없음 -> 사각 픽셀)
            Sz = solver.transmitted_flux_map(self.grid_ny, self.grid_nx).detach().cpu().numpy()
            for pidx in range(npx * npx):
                pix_abs[pidx] += Sz[pixidx == pidx].sum() / ngrid   # 심부 Si 흡수분 배분
            ...
            qe_total = float(A_band + T_deep)            # 전체 Si 흡수 (mean(픽셀QE)+trench)
            out["QE"] = qe_total
            out["A_stack"] = float(1.0 - o["R"] - qe_total)  # 상부(ARL/ML/CF/grid) 흡수 + 반사外 손실
            out["QE_pixels"] = qe_pix
            out["QE_rgb"] = qe_rgb
            out["QE_trench"] = float(trench_abs)         # DTI 트렌치 내부 흡수분 (픽셀 제외)
            out["QE_deep"] = float(T_deep)               # 그중 심부(밴드 아래) Si 흡수분 (참고)
        return out
