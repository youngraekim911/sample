...
def _feat_matrix(X):
    """X:(M,4) 스텝지수 -> 2차 특징 (M,15). x=step/2 정규화."""
    x = X / 2.0
    cols = [np.ones(len(X)), x[:, 0], x[:, 1], x[:, 2], x[:, 3],
...
def invert(surrogate, target, weights=None, channels=("R", "G", "B"),
    ...
    반환: {steps:[4], axes, predicted:{wl:{R,G,B}}, target, residual_rms,
           per_point:[...], at_edge:[bool×4], edge_warning}
    """
    axes = surrogate["axes"]
    ...
    # 1) 격자 스캔
    g = np.linspace(-bound, bound, grid)
    G = np.array(list(itertools.product(g, g, g, g)))       # (grid^4,4)
